...
import torch
from models.synthesis_hyper import Hyper_synthesis
...
class Entropy(nn.Module):
    ...
    def forward(self, sigma, y):
        ...
        scale0 = torch.clamp_min(torch.abs(scale0), 1e-4)
        scale1 = torch.clamp_min(torch.abs(scale1), 1e-4)
        scale2 = torch.clamp_min(torch.abs(scale2), 1e-4)
